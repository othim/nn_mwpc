...
for pp in mesh_points:
    ...
    max_inf, mean_inf = tt.run_tests_phase(obj_inf,tol)
    max_fin, mean_fin = tt.run_tests_phase(obj_fin,tol)
    print(mean_inf.shape)   
    max_inf_arr.append(max_inf)
    max_fin_arr.append(max_fin)
    mean_inf_arr.append(mean_inf)
    mean_fin_arr.append(mean_fin)
    print(f' # points: {pp}, max. rel. err. inf: {max_inf}, fin: {max_fin}')
    print(f' # points: {pp},mean. rel. err. inf: {mean_inf}, fin: {mean_fin}')
...
ax.plot(mesh_points,max_inf_arr,linestyle='--',marker='o',label='inf mesh, max')
ax.plot(mesh_points,max_fin_arr,linestyle='--',marker='o',label='fin mesh, max')
...
fig,ax = plt.subplots(1,1)
ax.plot(mesh_points,mean_inf_arr,linestyle='--',marker='o',label='inf mesh, mean')
ax.plot(mesh_points,mean_fin_arr,linestyle='--',marker='o',label='fin mesh, mean')
ax.set_xlabel('\# mesh points')
ax.set_ylabel('abs. rel. err')
ax.set_yscale('log')
ax.legend()
