...
the src/cpp_src directory to be accessed when compiling the C++ code.

2. The code is compiled by running the appropriate make command as defined in the 
...
'make obs' you can run the ./obs excecutable. You have to geive an argument.
Yuu can test phase shifts by runnning ./obs phase, or some differnent observables
...
it can be plotted with the python script plot_obs.py <OBS_NAME> in the
src/python_src directory.
